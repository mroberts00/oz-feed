class BusinessProfilesController < ApplicationController
  ...
  before_action :authenticate_business!, only: [:show, :edit, :update, :destroy]
  ...
  def create
    @business_profile = BusinessProfile.new(business_profile_params)
     @business_profile.business_id = current_business.id
  ...
  private
    ...
    def business_profile_params
      params.require(:business_profile).permit(:name, :about, :street_line_one, :street_line_two, :suburb, :postcode, :state, :country, :address_notes, :avatar, :abn, :contact_first_name, :contact_last_name, :phone)
    end
end
